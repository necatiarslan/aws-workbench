...
exports.DynamoDBQueryView = void 0;
/* eslint-disable @typescript-eslint/naming-convention */
const vscode = __importStar(require("vscode"));
const ui = __importStar(require("../common/UI"));
const api = __importStar(require("./API"));
class DynamoDBQueryView {
    static Current;
    panel;
    disposables = [];
    extensionUri;
    state;
    lastQueryParams;
    constructor(panel, extensionUri, region, tableName, tableDetails, selectedIndex) {
        this.panel = panel;
    ...
    static Render(extensionUri, region, tableName, tableDetails, selectedIndex) {
        ...
        const panel = vscode.window.createWebviewPanel("DynamoDBQueryView", `Query: ${tableName}`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    async executeQuery(params) {
        ...
        try {
            ...
            const result = await api.QueryTable(this.state.region, this.state.tableName, keyConditionExpression, expressionAttributeValues, selectedIndex || undefined, params.limit || 100);
    ...
    async openEditItem(item) {
        try {
            const { DynamoDBEditItemView } = await Promise.resolve().then(() => __importStar(require('./DynamoDBEditItemView')));
            DynamoDBEditItemView.Render(this.extensionUri, this.state.region, this.state.tableName, this.state.tableDetails, item, () => {
    ...
    async deleteItem(item) {
        try {
            const confirm = await vscode.window.showWarningMessage('Are you sure you want to delete this item?', { modal: true }, 'Delete');
            if (confirm !== 'Delete') {
                return;
            }
            const key = {};
    ...
    async openAddItem() {
        try {
            const { DynamoDBAddItemView } = await Promise.resolve().then(() => __importStar(require('./DynamoDBAddItemView')));
            DynamoDBAddItemView.Render(this.extensionUri, this.state.region, this.state.tableName, this.state.tableDetails, () => {
                if (this.lastQueryParams) {
                    this.executeQuery(this.lastQueryParams);
                }
            });
...
exports.DynamoDBQueryView = DynamoDBQueryView;
//# sourceMappingURL=DynamoDBQueryView.js.map