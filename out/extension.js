...
function registerCoreCommands(context, serviceManager, treeProvider, treeView) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aws-workbench.addResource', async () => {
    ...
    }), vscode.commands.registerCommand('aws-workbench.addResourceToFolder', async (folderNode) => {
        if (!folderNode || !folderNode.isFolder || !folderNode.itemData) {
            vscode.window.showWarningMessage('Please select a folder');
            return;
        }
        const folderId = folderNode.itemData.id;
        ...
        if (selected) {
            try {
                const node = await selected.service.addResource?.(folderId);
                treeProvider.refresh();
                if (node) {
                    setTimeout(() => {
                        treeView.reveal(node, { select: true, focus: true, expand: true });
    ...
    vscode.commands.registerCommand('aws-workbench.Filter', async () => {
        ...
        if (Session_1.Session.Current) {
            Session_1.Session.Current.toggleShowOnlyFavorites();
            treeProvider.refresh();
            const status = Session_1.Session.Current.isShowOnlyFavorites ? 'enabled' : 'disabled';
        ...
        if (Session_1.Session.Current) {
            Session_1.Session.Current.toggleShowHiddenNodes();
            treeProvider.refresh();
            const status = Session_1.Session.Current.isShowHiddenNodes ? 'enabled' : 'disabled';
            vscode.window.showInformationMessage(`Show Hidden Nodes ${status}`);
        }
    }), vscode.commands.registerCommand('aws-workbench.UpdateAwsEndPoint', () => Session_1.Session.Current?.SetAwsEndpoint()), vscode.commands.registerCommand('aws-workbench.SetAwsRegion', () => Session_1.Session.Current?.SetAwsRegion()), 
    ...
    vscode.commands.registerCommand('aws-workbench.CreateFolder', async () => {
    ...
    }), vscode.commands.registerCommand('aws-workbench.RenameFolder', async (node) => {
        if (!node.isFolder || !node.itemData)
            return;
    ...
    }), vscode.commands.registerCommand('aws-workbench.DeleteFolder', async (node) => {
        if (!node.isFolder || !node.itemData)
            return;
        const folder = node.itemData;
        const session = Session_1.Session.Current;
        if (!session)
            return;
        const descendantFolders = session.getDescendantFolders(folder.id);
        let resourceCount = 0;
        const serviceManager = ServiceManager_1.ServiceManager.Instance;
        for (const service of serviceManager.getAllServices()) {
            resourceCount += service.countResourcesInFolders([folder.id, ...descendantFolders]);
        }
        const confirmed = await vscode.window.showWarningMessage(`Delete folder "${folder.name}" and ${descendantFolders.length} subfolder(s) containing ${resourceCount} resource(s)?`, 'Delete', 'Cancel');
        if (confirmed === 'Delete') {
            await session.deleteFolder(folder.id);
            treeProvider.refresh();
            vscode.window.showInformationMessage(`Folder "${folder.name}" deleted`);
        }
    }), vscode.commands.registerCommand('aws-workbench.MoveResourceToFolder', async (node) => {
        if (node.isFolder || !node.compositeKey)
            return;
        const session = Session_1.Session.Current;
        if (!session)
            return;
        const folderId = await FolderManager_1.FolderManager.showFolderQuickPick(session);
        if (folderId === undefined)
            return; // User cancelled
        // Update custom resource's folder
        const service = serviceManager.getService(node.serviceId);
        const customResources = service.customResources;
        const resource = customResources.get(node.compositeKey);
        if (resource) {
            resource.folderId = folderId;
            await service.saveCustomResources();
            treeProvider.refresh();
    ...
    }), vscode.commands.registerCommand('aws-workbench.AddFolderToFav', (node) => {
        if (!node.isFolder || !node.itemData)
            return;
        const folder = node.itemData;
        Session_1.Session.Current?.addFolderToFavorites(folder.id);
        treeProvider.refresh();
        vscode.window.showInformationMessage(`Folder "${folder.name}" added to favorites`);
    }), vscode.commands.registerCommand('aws-workbench.RemoveFolderFromFav', (node) => {
        if (!node.isFolder || !node.itemData)
            return;
        const folder = node.itemData;
        Session_1.Session.Current?.removeFolderFromFavorites(folder.id);
        treeProvider.refresh();
        vscode.window.showInformationMessage(`Folder "${folder.name}" removed from favorites`);
    }), vscode.commands.registerCommand('aws-workbench.HideFolderNode', (node) => {
        if (!node.isFolder || !node.itemData)
            return;
        const folder = node.itemData;
        Session_1.Session.Current?.hideFolderNode(folder.id);
        treeProvider.refresh();
        vscode.window.showInformationMessage(`Folder "${folder.name}" hidden`);
    }), vscode.commands.registerCommand('aws-workbench.UnHideFolderNode', (node) => {
        if (!node.isFolder || !node.itemData)
            return;
        const folder = node.itemData;
        Session_1.Session.Current?.unhideFolderNode(folder.id);
        treeProvider.refresh();
        vscode.window.showInformationMessage(`Folder "${folder.name}" unhidden`);
    }), vscode.commands.registerCommand('aws-workbench.RemoveFromFolder', async (node) => {
        if (!node.isCustom || !node.compositeKey)
            return;
        const service = serviceManager.getService(node.serviceId);
        const customResources = service.customResources;
        const resource = customResources.get(node.compositeKey);
    ...
    vscode.commands.registerCommand('aws-workbench.FilterByFolder', async () => {
        ...
        if (!session)
            return;
        const folderId = await FolderManager_1.FolderManager.showFilterFolderQuickPick(session);
        if (folderId === undefined)
            return; // User cancelled
        ...
        if (!session)
            return;
        const filter = await FolderManager_1.FolderManager.showFilterResourceNameInput(session.resourceNameFilter);
        if (filter === undefined)
            return; // User cancelled
