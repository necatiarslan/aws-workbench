...
exports.DynamodbTreeView = void 0;
...
class DynamodbTreeView {
    ...
    LoadState() {
        ...
        try {
            let DynamodbListTemp = this.context.globalState.get('DynamodbList');
            if (DynamodbListTemp) {
                this.DynamodbList = DynamodbListTemp;
            }
    ...
    async AddDynamodb() {
        ...
        if (!selectedDynamodbList || selectedDynamodbList.length === 0) {
            return;
        }
    ...
    async RemoveDynamodb(node) {
        ...
        if (node.TreeItemType !== DynamodbTreeItem_1.TreeItemType.Dynamodb) {
            return;
        }
    ...
    async CreateTable() {
        ...
        // Create the table
        let result = await api.CreateDynamodbTable(region, tableName, partitionKeyName, partitionKeyType, sortKeyName, sortKeyType);
        if (result.isSuccessful) {
            // Add to tree view
    ...
    async DeleteTable(node) {
        ...
        // Confirm deletion
        let confirm = await vscode.window.showWarningMessage(`Are you sure you want to DELETE table "${node.Dynamodb}" in ${node.Region}? This action CANNOT be undone!`, { modal: true }, 'Delete Table');
        ...
        if (doubleConfirm !== node.Dynamodb) {
            return;
        }
        // Delete the table
    ...
    async EditCapacity(node) {
        ...
        if (!tableDetails.isSuccessful) {
            return;
        }
        let details = api.ExtractTableDetails(tableDetails.result);
        // Ask billing mode
        let billingMode = await vscode.window.showQuickPick(['PAY_PER_REQUEST (On-Demand)', 'PROVISIONED'], {
        ...
        let result = await api.UpdateTableCapacity(node.Region, node.Dynamodb, readCapacity, writeCapacity, mode);
        if (result.isSuccessful) {
            this.Refresh();
        }
    }
    async QueryTable(node) {
        ...
        const { QueryTablePanel } = await Promise.resolve().then(() => require('./QueryTablePanel'));
        await QueryTablePanel.createOrShow(this.context.extensionUri, node.Region, node.Dynamodb, details);
    }
    async ScanTable(node) {
        ...
        const { ScanTablePanel } = await Promise.resolve().then(() => require('./ScanTablePanel'));
        await ScanTablePanel.createOrShow(this.context.extensionUri, node.Region, node.Dynamodb, details);
    }
    async AddItem(node) {
        ...
        const { AddItemPanel } = await Promise.resolve().then(() => require('./AddItemPanel'));
        await AddItemPanel.createOrShow(this.context.extensionUri, node.Region, node.Dynamodb, details);
    }
    async EditItem(node) {
        ...
        try {
            ...
            ui.showErrorMessage('Invalid JSON format for expression values', error);
            return;
        }
        // Update the item
        let result = await api.UpdateItem(node.Region, node.Dynamodb, key, updateExpression, expressionAttributeValues);
        if (result.isSuccessful) {
    ...
    async DeleteItem(node) {
        ...
        // Confirm deletion
        let confirm = await vscode.window.showWarningMessage('Are you sure you want to delete this item?', { modal: true }, 'Delete Item');
    ...
    async ShowCapacityExplanation(node, capacityType) {
...
exports.DynamodbTreeView = DynamodbTreeView;
//# sourceMappingURL=DynamodbTreeView.js.map