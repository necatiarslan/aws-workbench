...
class IamTreeView {
    ...
    LoadState() {
        ...
        try {
            let IamRoleListTemp = this.context.globalState.get('IamRoleList');
            if (IamRoleListTemp) {
                this.IamRoleList = IamRoleListTemp;
            }
        }
        catch (error) {
    ...
    async RemoveIamRole(node) {
        ...
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.IamRole) {
            return;
        }
    ...
    async LoadPermissions(node) {
        ui.logToOutput('IamTreeView.LoadPermissions Started');
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.PermissionsGroup) {
            return;
        }
        if (node.IsRunning) {
            return;
        ...
        if (result.result && result.result.AttachedPolicies) {
            for (let policy of result.result.AttachedPolicies) {
                let policyNode = new IamTreeItem_1.IamTreeItem(policy.PolicyName || 'Unknown Policy', IamTreeItem_1.TreeItemType.Permission);
    ...
    async LoadTrustRelationships(node) {
        ui.logToOutput('IamTreeView.LoadTrustRelationships Started');
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.TrustRelationshipsGroup) {
            return;
        }
        if (node.IsRunning) {
            return;
            ...
            for (let statement of result.result.Statement) {
                if (statement.Principal && statement.Principal.Service) {
                    ...
                    for (let service of services) {
                        let trustNode = new IamTreeItem_1.IamTreeItem(`Service: ${service}`, IamTreeItem_1.TreeItemType.TrustRelationship);
    ...
    async LoadTags(node) {
        ...
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.TagsGroup) {
            return;
        }
        if (node.IsRunning) {
            return;
        ...
        if (result.result && result.result.Tags) {
            for (let tag of result.result.Tags) {
                let tagNode = new IamTreeItem_1.IamTreeItem(`${tag.Key} = ${tag.Value}`, IamTreeItem_1.TreeItemType.Tag);
    ...
    async LoadInfo(node) {
        ...
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.InfoGroup) {
            return;
        }
        if (node.IsRunning) {
            return;
        ...
        if (role) {
            ...
            for (let item of infoItems) {
                let infoNode = new IamTreeItem_1.IamTreeItem(`${item.key}: ${item.value}`, IamTreeItem_1.TreeItemType.InfoItem);
    ...
    async AddTag(node) {
        ...
        this.SetNodeRunning(node, true);
        let result = await api.AddIamRoleTag(node.Region, node.IamRole, tagKey, tagValue);
        if (!result.isSuccessful) {
    ...
    async UpdateTag(node) {
        ...
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.Tag) {
            return;
        }
        ...
        this.SetNodeRunning(node, true);
        let result = await api.UpdateIamRoleTag(node.Region, node.IamRole, node.TagKey, newValue);
        if (!result.isSuccessful) {
    ...
    async RemoveTag(node) {
        ...
        const confirm = await vscode.window.showWarningMessage(`Are you sure you want to remove tag "${node.TagKey}"?`, { modal: true }, 'Yes', 'No');
        if (confirm !== 'Yes') {
            return;
        }
        this.SetNodeRunning(node, true);
        let result = await api.RemoveIamRoleTag(node.Region, node.IamRole, node.TagKey);
        if (!result.isSuccessful) {
    ...
    async ViewPolicy(node) {
        ui.logToOutput('IamTreeView.ViewPolicy Started');
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.Permission) {
            return;
        }
        if (!node.PolicyArn) {
    ...
    async ViewTrustRelationship(node) {
        ui.logToOutput('IamTreeView.ViewTrustRelationship Started');
        if (node.TreeItemType !== IamTreeItem_1.TreeItemType.TrustRelationship) {
            return;
        }
    ...
    async DownloadTrustRelationship(node) {
...
exports.IamTreeView = IamTreeView;
//# sourceMappingURL=IamTreeView.js.map